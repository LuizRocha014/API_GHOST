...
/// pela tabela ABASTA_HistoryMigration. Cada script só é executado uma vez.
/// </summary>
public sealed class MigrationRunner
{
    private const string PrimaryConnection = "AbastaConnection";
    private const string FallbackConnection = "DefaultConnection";
    ...
    {
        var cs = _configuration.GetConnectionString(PrimaryConnection)
                 ?? _configuration.GetConnectionString(FallbackConnection)
                 ?? throw new InvalidOperationException(
                     $"ConnectionStrings:{PrimaryConnection} não configurado.");
    ...
    {
        const string sql = """
            IF OBJECT_ID(N'dbo.ABASTA_HistoryMigration', N'U') IS NULL
            BEGIN
                CREATE TABLE dbo.ABASTA_HistoryMigration
                (
                    id           INT             NOT NULL IDENTITY(1,1) CONSTRAINT PK_ABASTA_HistoryMigration PRIMARY KEY,
                    name         NVARCHAR(255)   NOT NULL,
                    executed_at  DATETIME2(3)    NOT NULL CONSTRAINT DF_ABASTA_HistoryMigration_executed DEFAULT (SYSUTCDATETIME()),
                    CONSTRAINT UQ_ABASTA_HistoryMigration_name UNIQUE (name)
                );
            END
            """;
    ...
    {
        const string sql = "SELECT name FROM dbo.ABASTA_HistoryMigration";
    ...
    {
        const string sql = "INSERT INTO dbo.ABASTA_HistoryMigration (name) VALUES (@Name)";
