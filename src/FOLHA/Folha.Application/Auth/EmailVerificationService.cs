...
public sealed class EmailVerificationService : IEmailVerificationService
{
    ...
    public async Task SendCodeAsync(User user, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Não propaga: o código já está salvo e o usuário pode pedir reenvio.
    ...
    public async Task<(EmailVerificationStatus Status, User? User)> VerifyAsync(string email, string code, CancellationToken cancellationToken = default)
    {
        ...
        // Idempotente: já verificado é sucesso.
    ...
    private static string GenerateCode(int length)
    {
        ...
        return value.ToString(System.Globalization.CultureInfo.InvariantCulture).PadLeft(length, '0');
    }
    ...
    private (string Subject, string Html, string Text) BuildEmail(User user, string code)
    {
        ...
        var greeting = string.IsNullOrEmpty(name) ? "Olá!" : $"Olá, {name}!";
        const string subject = "Seu código de verificação Folha";
        var ttl = _options.CodeTtlMinutes;

        var text =
            $"{greeting}\n\n" +
            $"Seu código de verificação é: {code}\n\n" +
            $"Ele expira em {ttl} minutos. Se você não criou uma conta na Folha, ignore este e-mail.\n\n" +
            "— Folha";
        ...
        var html =
            ...
            <!DOCTYPE html>
            <html lang="pt-BR">
            <body style="margin:0;padding:0;background-color:#F3F1E9;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#F3F1E9;padding:32px 0;">
                <tr>
                  <td align="center">
                    <table role="presentation" width="440" cellpadding="0" cellspacing="0" style="background-color:#FFFFFF;border-radius:16px;overflow:hidden;border:1px solid #E4E0D2;">
                      <tr>
                        <td style="padding:32px 36px 8px 36px;">
                          <p style="margin:0;font-size:14px;letter-spacing:2px;text-transform:uppercase;color:#5C7A4A;font-weight:600;">Folha</p>
                          <h1 style="margin:16px 0 8px 0;font-size:26px;font-style:italic;color:#2F4A23;">Confirme seu e-mail</h1>
                          <p style="margin:0;font-size:15px;line-height:1.6;color:#4A4A40;">{{greeting}} Use o código abaixo para confirmar sua conta na Folha.</p>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding:24px 36px;">
                          <div style="background-color:#EEF3E8;border:1px solid #D6E2C9;border-radius:12px;padding:20px;text-align:center;">
                            <span style="font-size:36px;font-weight:700;letter-spacing:10px;color:#2F4A23;">{{code}}</span>
                          </div>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding:0 36px 32px 36px;">
                          <p style="margin:0;font-size:13px;line-height:1.6;color:#7A7A6E;">O código expira em {{ttl}} minutos. Se você não criou uma conta na Folha, é só ignorar este e-mail.</p>
                        </td>
                      </tr>
                    </table>
                  </td>
